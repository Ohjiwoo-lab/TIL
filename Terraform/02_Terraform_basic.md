...
이미 존재하는 `terraform101-inflearn-jiwoo` 버킷을 다시 생성한다고 예측하였다. 여기서 알 수 있는 것은 테라폼은 실제 인프라가 존재하는 지 여부를 확인하지 않는다는 것이다. 위의 경우에서는 백엔드에 state 파일이 저장되어 있는 지를 확인하고 없기 때문에 새롭게 만들어야겠다고 판단한 것이다.

즉, 테라폼이 인프라를 인식하는 방법은 state 파일인 것이다.
...
결국 테라폼은 인프라의 정보를 state 파일로 관리한다는 것을 알았다. state 파일이 있다면 인프라가 존재하는 것으로 판단한다.

그러면 이미 존재하는 인프라의 상태 정보를 불러오면 테라폼이 해당 인프라를 인식하게 할 수 있다.
